...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	///TODO: implement the following function as described in Output.h file
	int v = cellPos.VCell();
	int y = UI.ToolBarHeight + v * UI.CellHeight;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_Edit] = "images\\Menu_Edit.jpg";
	...
	// item Order add laddr - add snake-- add card -- copy --cut ---paste--edit --delete-- save-- open 
...
void Output::CreatePlayModeToolBar() const
{
	...
	///TODO: Prepare images for each menu item and add it to the list
	// order roll dice -- input dice value -- new game -- switch to design mode -- exit
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	//       (Use GetIntegerSize() window function) and set the "w" and "h" variables with its width and height
	int cnum = 0;
	cnum = cellPos.GetCellNum();
	pWind->GetIntegerSize(w, h, cnum);
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.HCell() != toCell.HCell() || fromCell.GetCellNum() > toCell.GetCellNum())
		return;
	...
	int x = cellStartX + UI.CellWidth - UI.LadderXOffset; // same x (vertical line)
	...
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
	int countverticalcells = fromCell.VCell() - toCell.VCell();
	for (int i = 0; i < countverticalcells; i++)
	{
		pWind->DrawLine(x12, (y2 + (UI.CellHeight / 2) + (i * (UI.CellHeight))), x, (y2 + (UI.CellHeight / 2) + (i * (UI.CellHeight))));
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int PX[4] = { x12,x12 - xChange,x12,x12 + xChange };
	int PY[4] = { y1 + 10,y1 + 10 - yChange,(y1 + 10 - (2 * yChange)),y1 + 10 - yChange };

	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way

	pWind->DrawPolygon(PX, PY, 4);
	// done but Question in +10
}
